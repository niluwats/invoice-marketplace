...
type BidRepository interface {
	ProcessBid(bid domain.Bid, restBalance float64) *appErr.AppError
...
func (repo BidRepositoryDb) ProcessBid(bid domain.Bid, restBalance float64) *appErr.AppError {
	...
	if err != nil {
		return appErr.NewUnexpectedError("Error inserting bid : " + err.Error())
	}
	...
	if err != nil {
		return appErr.NewUnexpectedError("Error updating investor's balance : " + err.Error())
	}
	...
	if restBalance <= bid.BidAmount {
		...
		if err != nil {
			return appErr.NewUnexpectedError("Error updating invoice lock status : " + err.Error())
		}
	}
